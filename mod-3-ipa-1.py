...
def vigenere_cipher(message, key):
    ...
    If needed, the keyphrase is extended to match the length of the key.
        If key is "KEY" and the message is "LONGTEXT",
...
def scytale_cipher(message, shift):
    ...
    if (len(message) % shift) == 0:
        ...
        for i in range(len(message)):
            ...
            jumbled_message.append(message[x])
        return("".join(jumbled_message))    
    else:
        jumbled_message = []
        message = list(message)
        y = shift - (len(message) % shift)
        message.extend(itertools.repeat('_', y))
        for i in range(len(message)):
            x = (i // shift) + (len(message) // shift) * (i % shift)
            jumbled_message.append(message[x])
        return("".join(jumbled_message))
...
def scytale_decipher(message, shift):
    ...
    k = 0
    while k < shift:
        for i in range(k, len(message), shift):
            before_message.append(message[i])
        k += 1
    return("".join(before_message))